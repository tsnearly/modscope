...
export const POST_DETAIL_ICONS = {
    ...
    engagement: {
        screen: 'mono-score.png',
        printed: 'mono-score.png',
    },
    depth: {
...
export const DATA_GROUPING_ICONS = {
    ...
    top_influencer: {
        screen: 'mono-staring-contest-alt.png',
...
const iconAssets = import.meta.glob('../../assets/**/*.{png,svg}', { eager: true, as: 'url' });
...
export function getIconPath(filename: string): string {
    if (!filename) return '';
    if (filename.startsWith('lucide:')) return filename;
    ...
    if (!match) {
        match = keys.find(key => {
            ...
            return k.endsWith(`/${normalized.toLowerCase()}`) ||
    ...
    if (!match) {
        match = keys.find(key => key.toLowerCase().includes(basename.toLowerCase()));
    }
