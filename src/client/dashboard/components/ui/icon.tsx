...
const iconVariants = cva(
    'inline-block object-contain flex-shrink-0',
    {
        variants: {
            size: {
                xxs: 'h-2 w-2',
...
export interface IconProps extends Omit<React.ImgHTMLAttributes<HTMLImageElement>, 'size'> {
    name?: string;
    src?: string;
...
export function Icon({ name, src, size = 'sm', variant, color, className, alt = '', style, ...props }: IconProps) {
    // ── Lucide library path: "lucide:chevron-down" ──────────────────────────
    const lucideRef = name?.startsWith('lucide:') ? name : src?.startsWith('lucide:') ? src : undefined;
    if (lucideRef) {
        const iconName = lucideRef.slice(7); // strip "lucide:"
        const componentName = toPascalCase(iconName);
        const LucideIcon = (LucideIcons as unknown as Record<string, React.ComponentType<{ size?: number; color?: string; className?: string; style?: React.CSSProperties }> | undefined>)[componentName];

        if (!LucideIcon) {
            console.warn(`[Icon] Lucide icon not found: "${componentName}" (from "${name}")`);
            return null;
        }

        const pxSize = typeof size === 'number' ? size : { xxs: 8, xs: 12, sm: 16, md: 20, lg: 24, xl: 32 }[size ?? 'sm'] ?? 16;
        return (
            <LucideIcon
                size={pxSize}
    ...
    if (color) {
        ...
        if (typeof size === 'number') {
            return <div className={cn('flex-shrink-0', className)} style={{ width: size, height: size, ...maskedStyle }} />;
        }
        return <div className={cn(iconVariants({ size: size as any }), className)} style={maskedStyle} />;
    }
