...
export function TimePicker({
    ...
    const handleHourChange = (newHourStr: string) => {
        ...
        setTempTime(`${finalHour.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`);
    };
    ...
    const handleAmPmChange = (ampm: 'AM' | 'PM') => {
        ...
        setTempTime(`${newHour.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`);
    };
    ...
    const handlePointerMove = (e: React.PointerEvent<SVGSVGElement>) => {
        ...
        if (isDragging === 'hour') {
            ...
            setTempTime(`${newH24.toString().padStart(2, '0')}:${minutes.toString().padStart(2, '0')}`);
            ...
            setTempTime(`${hours.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}`);
        }
    };
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent className="sm:max-w-[350px] flex flex-col items-center bg-[#1f2329] border-[#2a2e35] text-slate-100">
                    <DialogHeader className="w-full">
                        <DialogTitle className="text-center pb-4 border-b border-[#2a2e35] text-xl">Set Time</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="relative w-64 h-64 mt-6 mb-2">
                        ...
                        <div className="absolute inset-0 rounded-full border-[6px] border-[#2a2e35] bg-white shadow-2xl flex items-center justify-center overflow-hidden">
                            ...
                            >
                                ...
                                <text x="100" y="60" textAnchor="middle" className="text-[10px] fill-slate-300 font-bold tracking-widest">MODSCOPE</text>
                                ...
                                <g transform={`rotate(${hourAngle}, ${center}, ${center})`}>
                                    {/* Shadow */}
                                    <line x1={center + 2} y1={center + 2} x2={center + 2} y2={center - hourHandLength + 2} stroke="rgba(0,0,0,0.2)" strokeWidth="6" strokeLinecap="round" />
                                    {/* Hand */}
                                    <line x1={center} y1={center} x2={center} y2={center - hourHandLength} stroke="currentColor" strokeWidth="6" strokeLinecap="round" />
                                </g>

                                {/* Minute Hand */}
                                <g transform={`rotate(${minuteAngle}, ${center}, ${center})`}>
                                    {/* Shadow */}
                                    <line x1={center + 2} y1={center + 2} x2={center + 2} y2={center - minuteHandLength + 2} stroke="rgba(0,0,0,0.1)" strokeWidth="4" strokeLinecap="round" />
                                    {/* Hand */}
                                    <line x1={center} y1={center} x2={center} y2={center - minuteHandLength} stroke="#000000ff" strokeWidth="4" strokeLinecap="round" />
                                    <circle cx={center} cy={center - minuteHandLength} r="2" fill="#000000ff" />
                                </g>
                                ...
                                <circle
                                    cx={center + Math.sin(hourAngle * Math.PI / 180) * hourHandLength}
                                    cy={center - Math.cos(hourAngle * Math.PI / 180) * hourHandLength}
                                    r="15" fill="transparent" cursor="grab"
                                />
                                <circle
                                    cx={center + Math.sin(minuteAngle * Math.PI / 180) * minuteHandLength}
                                    cy={center - Math.cos(minuteAngle * Math.PI / 180) * minuteHandLength}
                                    r="15" fill="transparent" cursor="grab"
                                />
                    ...
                    <div className="flex items-center gap-2 mt-6 w-full justify-center bg-slate-100 p-4 rounded-xl border border-slate-200">
                        {/* Hour */}
                        <Select value={displayHours.toString()} onValueChange={handleHourChange}>
                            {/* Explicit Light Text Colors for these inputs since they are on white bg */}
                            <SelectTrigger className="w-[70px] bg-white border border-slate-200 shadow-sm font-mono text-lg font-bold text-center justify-center text-slate-900 focus:ring-slate-400">
                                <SelectValue placeholder="HH" />
                            </SelectTrigger>
                            <SelectContent className="max-h-[200px] bg-white text-slate-900 border-slate-200">
                                {Array.from({ length: 12 }, (_, i) => i + 1).map(h => (
                                    <SelectItem key={h} value={h.toString()} className="focus:bg-slate-100 focus:text-slate-900">{h.toString().padStart(2, '0')}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                        ...
                        <Select value={minutes.toString().padStart(2, '0')} onValueChange={handleMinuteChange}>
                            ...
                            <SelectContent className="max-h-[200px] bg-white text-slate-900 border-slate-200">
                                {Array.from({ length: 60 }, (_, i) => i).map(m => (
                                    <SelectItem key={m} value={m.toString().padStart(2, '0')} className="focus:bg-slate-100 focus:text-slate-900">
                        ...
                        {/* AM/PM */}
                        <Select value={isPM ? 'PM' : 'AM'} onValueChange={(val) => handleAmPmChange(val as 'AM' | 'PM')}>
                            <SelectTrigger className="w-[70px] bg-white border border-slate-200 shadow-sm font-mono text-lg font-bold text-center justify-center text-slate-900 focus:ring-slate-400">
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="bg-white text-slate-900 border-slate-200">
                                <SelectItem value="AM" className="focus:bg-slate-100 focus:text-slate-900">AM</SelectItem>
                                <SelectItem value="PM" className="focus:bg-slate-100 focus:text-slate-900">PM</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <DialogFooter className="w-full mt-6">
                        <div className="flex w-full gap-2">
                            <DialogClose asChild>
                                <Button className="flex-1 hover:bg-[#2a2e35] hover:text-white">Cancel</Button>
                            </DialogClose>
                            <Button onClick={handleSave} className="flex-1 bg-blue-600 text-white hover:bg-blue-500 border-0 h-11 text-base font-semibold shadow-lg shadow-blue-900/20">Set Time</Button>
                        </div>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
