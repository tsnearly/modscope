...
export function TabList({ children, className = '', ...props }: TabListProps) {
    ...
    if (!context) throw new Error('TabList must be used within TabsGroup');
...
export function Tab({ value, name, icon, iconTrailing, accent, children, className = '', ...props }: TabProps) {
    const context = useContext(TabsContext);
    if (!context) throw new Error('Tab must be used within TabsGroup');

    const isActive = context.value === value;
    const { variant, size } = context;

    const baseStyles = "inline-flex items-center justify-center gap-2 transition-all focus:outline-none disabled:opacity-50 disabled:pointer-events-none";
    ...
    const variantStyles = {
        ...
        pills: cn(
            ...
            isActive
                ? (accent ? `bg-${accent}-500 text-white border-${accent}-500` : "bg-primary text-primary-foreground border-primary")
...
export function TabPanel({ value, children, className = '', ...props }: TabPanelProps) {
    const context = useContext(TabsContext);
    if (!context) throw new Error('TabPanel must be used within TabsGroup');
