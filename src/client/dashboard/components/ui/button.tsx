    ...
    "inline-flex items-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
    {
        variants: {
            variant: {
                ...
                subtle: "bg-blue-50/50 text-blue-700 hover:bg-blue-50 active:bg-blue-100/50",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({ className, variant, size, align, asChild = false, icon, iconVariant = "micro", iconSize, iconTrailing, square, loading, tooltip, tooltipPosition = 'top', tooltipBd, kbd, inset, children, ...props }, ref) => {
        const Comp = asChild ? Slot : "button"

        // Determine icon size based on iconVariant
        const getIconSize = () => {
            if (iconSize) return iconSize
            if (size === 'icon') return 22
            if (size === 'xs') return 12
            switch (iconVariant) {
                case "outline": return 20
                case "solid": return 18
                case "mini": return 16
        ...
        const buttonContent = (
            <Comp
                className={cn(buttonVariants({ variant, size, align, className }), squareClass, insetClass)}
                ref={ref}
            ...
            >
                {loading ? (
                    ...
                    <>
                        {icon && <Icon name={icon} size={finalIconSize} className="flex-shrink-0" />}
                        {children && (
                            <span className={cn(
                        ...
                        {iconTrailing && <Icon name={iconTrailing} size={finalIconSize} className="flex-shrink-0" />}
                    </>
                )}
            </Comp>
        )
        ...
        if (tooltip) {
            return (
                <Tooltip content={tooltip} side={tooltipPosition} className={tooltipBd ? `bg-${tooltipBd}` : ''}>
                    {buttonContent}
                </Tooltip>
            )
        }
