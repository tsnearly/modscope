...
export function RadioGroup({
    ...
    const groupContent = (
        <RadioGroupContext.Provider value={{
            ...
            <div className={cn(
                variant === 'cards' ? "grid grid-cols-1 md:grid-cols-2 gap-4" : "space-y-2",
                variant === 'segmented' ? "flex bg-muted p-1 rounded-lg self-start" : "",
                className
            )}>
                {children}
            </div>
...
export function RadioItem({ value, id, label, description, icon, disabled: ownDisabled, className = '' }: RadioGroupItemProps) {
                    ...
                    <div className="flex items-start gap-4">
                        {icon && (
                            <div className={cn(
                                ...
                                isChecked ? "bg-primary text-white border-primary" : "bg-gray-50 border-gray-100 text-gray-400"
                            )}>
                                <Icon name={icon} size={24} />
                            </div>
                        )}
                        <div>
                            <span className={cn("block text-sm font-bold", isChecked ? "text-primary" : "text-gray-900")}>
                                {label}
                            </span>
                            {description && (
    ...
    return (
        <label className={cn("flex items-start gap-3 cursor-pointer group", disabled && "opacity-50 cursor-not-allowed", className)}>
            ...
            <div className="text-sm">
                <span className={cn("font-medium transition-colors", isChecked ? "text-foreground" : "text-gray-700 group-hover:text-foreground")}>
                    {label}
                </span>
                {description && (
