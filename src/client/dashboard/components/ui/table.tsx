...
export const Table = React.forwardRef<HTMLDivElement, TableProps>(
    ({ children, className = '', containerClassName = '', containerStyle }, ref) => {
        return (
            <div ref={ref} className={`w-full overflow-x-auto relative rounded-md border border-gray-200 bg-white shadow-sm ${containerClassName}`} style={containerStyle}>
...
export function TableHead({ children, className = '', sortable, sortDirection, onSort, ...props }: TableHeadProps) {
            ...
            <div className="flex items-center gap-1">
                ...
                {sortable && (
                    <div className="flex flex-col opacity-50 ml-1">
                        {sortDirection === 'asc' ? (
                            <Icon name="lucide:chevron-up" size="xxs" className="opacity-100" />
                        ) : sortDirection === 'desc' ? (
                            <Icon name="lucide:chevron-down" size="xxs" className="opacity-100" />
                        ) : (
...
export function TableBody({ children, className = '', ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
    return (
        <tbody className={`[&_tr:nth-child(even)]:bg-gray-50/50 ${className}`} {...props}>
            {children}
        </tbody>
    );
}
...
export function TableCaption({ children, className = '', ...props }: React.ComponentProps<"caption">) {
    return (
        <caption
