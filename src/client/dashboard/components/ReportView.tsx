...
function ReportView({
    ...
    for (let d = 0; d < 7; d++) {
      for (let h = 0; h < 24; h++) {
        ...
        if (count > 0 && thresholds) {
          if (count >= thresholds!.superhigh[0]) {
            intensity = 5;
          } else if (count >= thresholds!.extreme[0]) {
            intensity = 4;
          } else if (count >= thresholds!.high[0]) {
            intensity = 3;
          } else if (count >= thresholds!.medium[0]) {
            intensity = 2;
          } else {
            intensity = 1;
          }
  ...
  useEffect(() => {
    if (!reportSettings.showTrendSubscribers) {
      return;
    }

    const fetchSnapshots = async () => {
      try {
        const currentReportDate = analytics?.meta?.scan_date
          ? new Date(analytics.meta.scan_date).getTime()
          : Date.now();
        const past =
          currentReportDate -
          (reportSettings.trendAnalysisDays || 90) * 24 * 60 * 60 * 1000;
        ...
        if (res.ok) {
          ...
          if (snapshotsArray.length > 0) {
            const filtered = snapshotsArray.filter((s: any) => {
              const t = new Date(s.scanDate).getTime();
              return t >= past && t <= currentReportDate;
            });
            setHistoricalSnapshots(
              filtered.sort(
                (a: any, b: any) =>
                  new Date(a.scanDate).getTime() -
                  new Date(b.scanDate).getTime(),
              ),
            );
          }
        }
    ...
    reportSettings.trendAnalysisDays,
    analytics?.meta?.scan_date,
  ]);

  // Load trends data when trends tab is active OR in print mode
  const trendsCacheKey = `${analytics?.meta?.subreddit || 'unknown'}:${analytics?.meta?.scan_date || 'unknown'}`;
  ...
  const renderTabContent = (tab: ReportTab) => {
    ...
    switch (tab) {
      case 'overview':
        ...
        const poolRaw = analytics?.analysis_pool;
      ...
      case 'content':
        const poolRawForContent = analytics?.analysis_pool;
      ...
      case 'activity':
        ...
        /*                // 1. Community Growth Data
                ...
                const getEngagementTrend = () => {
                    const posts = analytics.analysis_pool;
      ...
      {isPrintMode &&
        (() => {
          const printPool = excludeOfficial
            ? analytics.analysis_pool.filter(
                (p: any) =>
                  !effectiveOfficials.includes(p.author) &&
                  p.author !== officialAccount &&
                  p.author !== 'None',
                ...
                <div className="mb-8 border-b-4 border-slate-900 pb-4 flex justify-between items-end">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-slate-500">
                      Generated:{' '}
                      {analytics.meta?.scan_date || new Date().toLocaleString()}
                    </div>
                  </div>
                </div>
